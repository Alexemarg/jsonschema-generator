...
const createTypeDefsFile = require('./tsgen');
...
async function processFile(path, name) {
  ...
  const size = createJsonFile(to, name, res, isDemo || isDev);
  if (config.tsgen.enabled) {
    const jsonPath = `${to}/${name}.json`;
    await createTypeDefsFile(jsonPath, name);
  }
  return size;
}
